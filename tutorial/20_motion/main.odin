package main

import "core:fmt"
import "core:time"
import sdl "vendor:sdl3"
import "../../sutil/tex"
import "../../sutil/timer"

WIDTH :: 640
HEIGHT :: 480
SCREEN_FPS :: 60

// Dot constants
DOT_WIDTH :: 20
DOT_HEIGHT :: 20
DOT_VEL :: 10

// Dot struct
Dot :: struct {
    pos_x: i32,
    pos_y: i32,
    vel_x: i32,
    vel_y: i32,
}

// Global variables
renderer: ^sdl.Renderer
window: ^sdl.Window
dot_texture: ^tex.Texture

// Dot procedures
dot_init :: proc(dot: ^Dot) {
    dot.pos_x = 0
    dot.pos_y = 0
    dot.vel_x = 0
    dot.vel_y = 0
}

dot_handle_event :: proc(dot: ^Dot, e: ^sdl.Event) {
    // If a key was pressed (fix: check for !repeat to handle initial press)
    if e.type == sdl.EventType.KEY_DOWN && !e.key.repeat {
        // Adjust the velocity
        switch e.key.key {
        case sdl.K_UP:
            dot.vel_y -= DOT_VEL
        case sdl.K_DOWN:
            dot.vel_y += DOT_VEL
        case sdl.K_LEFT:
            dot.vel_x -= DOT_VEL
        case sdl.K_RIGHT:
            dot.vel_x += DOT_VEL
        }
    } else if e.type == sdl.EventType.KEY_UP && !e.key.repeat {
        // If a key was released adjust the velocity
        switch e.key.key {
        case sdl.K_UP:
            dot.vel_y += DOT_VEL
        case sdl.K_DOWN:
            dot.vel_y -= DOT_VEL
        case sdl.K_LEFT:
            dot.vel_x += DOT_VEL
        case sdl.K_RIGHT:
            dot.vel_x -= DOT_VEL
        }
    }
}

dot_move :: proc(dot: ^Dot) {
    // Move the dot left or right
    dot.pos_x += dot.vel_x
    // If the dot went too far to the left or right
    if dot.pos_x < 0 || dot.pos_x + DOT_WIDTH > WIDTH {
        // Move back
        dot.pos_x -= dot.vel_x
    }
    // Move the dot up or down
    dot.pos_y += dot.vel_y
    // If the dot went too far up or down
    if dot.pos_y < 0 || dot.pos_y + DOT_HEIGHT > HEIGHT {
        // Move back
        dot.pos_y -= dot.vel_y
    }
}

dot_render :: proc(renderer: ^sdl.Renderer, dot: ^Dot) {
    if dot_texture != nil {
        tex.render_rect(renderer, dot_texture, f32(dot.pos_x), f32(dot.pos_y))
    }
}

init :: proc() -> bool {
    if !sdl.Init(sdl.INIT_VIDEO) {
        fmt.eprintfln("SDL could not init! SDL error: %s", sdl.GetError())
        return false
    }
    if !sdl.CreateWindowAndRenderer(
        "SDL3 Tutorial: Motion",
        WIDTH, HEIGHT, {},
        &window, &renderer
    ) {
        fmt.eprintfln("Window could not be created! SDL error: %s", sdl.GetError())
        return false
    }
    return true
}

load_media :: proc() -> bool {
    // Allocate memory for the texture
    dot_texture = new(tex.Texture)
    if ok := tex.from_img(renderer, dot_texture, "../dot.png"); !ok {
        fmt.eprintln("Failed to load dot image!")
        return false
    }
    return true
}

main :: proc() {
    exit_code: int
    if !init() {
        fmt.eprintln("Failed to init program!")
        exit_code = 1
    } else {
        if !load_media() {
            fmt.eprintln("Failed to load media!")
            exit_code = 2
        } else {          
            // Timer to cap frame rate
            cap_timer: timer.Timer
            // Dot we will be moving around on screen
            dot: Dot
            dot_init(&dot)
            e: sdl.Event
            quit: bool  
            for !quit {
                timer.start(&cap_timer)
                for sdl.PollEvent(&e) {
                    #partial switch e.type {
                    case .QUIT:
                        quit = true
                    case .KEY_DOWN:
                        switch e.key.key {
                        case sdl.K_ESCAPE:
                            quit = true
                        }
                    }
                    // Process dot events
                    dot_handle_event(&dot, &e)
                }
                dot_move(&dot)
                // Fill the background
                sdl.SetRenderDrawColor(renderer, 0xFF, 0xFF, 0xFF, 0xFF)
                sdl.RenderClear(renderer)

                dot_render(renderer, &dot)
                // Update screen
                sdl.RenderPresent(renderer)
                // Cap frame rate
                timer.cap(&cap_timer, SCREEN_FPS)
            }
        }
    }
    close()
}

close :: proc() {
    if dot_texture != nil {
        tex.destroy(dot_texture)
        free(dot_texture)
    }
    if renderer != nil {
        sdl.DestroyRenderer(renderer)
        renderer = nil
    }
    if window != nil {
        sdl.DestroyWindow(window)
        window = nil
    }
    sdl.Quit()
}